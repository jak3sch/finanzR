#' Split a date-time column in two columns
#'
#' @description Splits a date-timecolumn in two separate columns.
#'
...
#' )
#'
#' r <- split_datetime(kraken_data)
#' head(r, 10)

split_datetime <- function(input, colname) {
  input <- input %>%
    dplyr::mutate(time = stringr::str_split({{colname}}, " ")) %>%
